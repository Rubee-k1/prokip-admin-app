...
interface Commission {
    id: string;
    agent: string;
    role: 'Agent' | 'Manager' | 'Telesales'; // Extended
...
const AdminCommissionsView: React.FC = () => {
    ...
    const [commissions, setCommissions] = useState<Commission[]>([
        ...
        { id: '7', agent: 'Mary J.', role: 'Telesales', business: 'Benin Bakers', amount: '₦40,000', status: 'Approved', date: 'Oct 19, 2023', type: 'New Sale', detail: 'Standard Plan', ref: 'TXN-007' },
        { id: '8', agent: 'Mary J.', role: 'Telesales', business: 'Enugu Electronics', amount: '₦20,000', status: 'Pending', date: 'Oct 18, 2023', type: 'Add-on', detail: 'SMS Integration', ref: 'TXN-008' },
    ]);
    ...
    const [pendingInvoices, setPendingInvoices] = useState<PendingInvoice[]>([
        ...
        },
        {
            ...
            agentName: 'Mary J.',
            role: 'Telesales',
            recipientName: 'Benin Bakers',
            amount: 80000,
            date: '2023-10-29',
            status: 'Unpaid',
            items: [
                { description: 'Standard Plan (3 Months)', quantity: 1, price: 80000, type: 'Plan' }
            ]
        }
    ]);
    ...
    const [walletTransactions] = useState<WalletTransaction[]>([
        ...
        { id: 'WT-5', user: 'Mary J.', role: 'Telesales', type: 'Credit', amount: '₦40,000', description: 'Commission payout for Standard Plan (Benin Bakers)', date: 'Oct 19, 2023', status: 'Completed', ref: 'MNFY-WT005' },
        { id: 'WT-6', user: 'Mary J.', role: 'Telesales', type: 'Debit', amount: '₦15,000', description: 'Bank Withdrawal', date: 'Oct 18, 2023', status: 'Completed', ref: 'MNFY-WT006' }
    ]);
    ...
    // Mock balances 
    const totalTelesalesBalance = 3200000;
        ...
        <div className="p-6 animate-fade-in space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col justify-center">
                    ...
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">Telesales Balance</p>
                    <h3 className="text-2xl font-extrabold text-slate-800">₦{totalTelesalesBalance.toLocaleString()}</h3>
                </div>

                {/* 3. Agents (BRM) Balances */}
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col justify-center">
                    <div className="flex justify-between items-start mb-2">
                    ...
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">Agent (BRM) Balance</p>
                    <h3 className="text-2xl font-extrabold text-slate-800">₦{totalAgentsBalance.toLocaleString()}</h3>
                </div>

                {/* 4. State Managers Balances */}
                <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col justify-center">
                    <div className="flex justify-between items-start mb-2">
                    ...
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mb-1">State Manager Balance</p>
            ...
            <div className="flex border-b border-slate-200 justify-between items-center">
                ...
                <div className="flex gap-2 mb-1 mt-2 sm:mt-0 flex-wrap sm:flex-nowrap justify-end items-center">
                    ...
                    <select 
                        className="bg-white border border-slate-200 text-xs rounded-lg px-2 py-1.5 outline-none focus:border-[#02275A] shadow-sm font-semibold text-slate-700"
                        value={roleFilter}
                        onChange={(e) => setRoleFilter(e.target.value)}
                    >
                        <option value="All">All Roles</option>
                        <option value="Telesales">Telesales</option>
                        <option value="Agent">Agent (BRM)</option>
                                    ...
                                    <tr key={comm.id} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="p-4">
                                            <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${
                                                comm.role === 'Manager' ? 'bg-indigo-100 text-indigo-700' :
                                                comm.role === 'Telesales' ? 'bg-amber-100 text-amber-700' :
                                                'bg-emerald-100 text-emerald-700'
                                            }`}>
                                                {comm.role === 'Manager' ? 'State Manager' :
                                                 comm.role === 'Agent' ? 'Agent (BRM)' :
                                                 comm.role}
                                            </span>
                                        </td>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredInvoices.map(inv => (
                                    <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${
                                                inv.role === 'Manager' ? 'bg-indigo-100 text-indigo-700' :
                                                inv.role === 'Telesales' ? 'bg-amber-100 text-amber-700' :
                                                'bg-emerald-100 text-emerald-700'
                                            }`}>
                                                {inv.role === 'Manager' ? 'State Manager' :
                                                 inv.role === 'Agent' ? 'Agent (BRM)' :
                                                 inv.role}
                                            </span>
                                        </td>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredWalletTransactions.map(txn => (
                                    <tr key={txn.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${
                                                txn.role === 'Manager' ? 'bg-indigo-100 text-indigo-700' :
                                                txn.role === 'Telesales' ? 'bg-amber-100 text-amber-700' :
                                                'bg-emerald-100 text-emerald-700'
                                            }`}>
                                                {txn.role === 'Manager' ? 'State Manager' :
                                                 txn.role === 'Agent' ? 'Agent (BRM)' :
                                                 txn.role}
                                            </span>
                                        </td>
                                        <td className="p-4">
