...
const AdminResourceCenterView: React.FC<AdminResourceCenterViewProps> = ({ contentItems, setContentItems, kbModules, setKbModules }) => {
    const { showSuccess, showError } = useAlert();
    const [viewTab, setViewTab] = useState<'content' | 'kb'>('content');
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-2xl shadow-xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h2 className="text-xl font-bold text-slate-800">
                                {editingContentItem ? 'Edit Marketing Content' : 'Add New Content'}
                            </h2>
                            ...
                            <form id="contentForm" onSubmit={handleSaveContent} className="space-y-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="md:col-span-2">
                                        ...
                                        <input type="text" required value={cTitle} onChange={(e) => setCTitle(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Description *</label>
                                        <textarea required value={cDescription} onChange={(e) => setCDescription(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A] min-h-[80px]" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Content Type *</label>
                                        <select value={cType} onChange={(e) => setCType(e.target.value as any)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]">
                                    ...
                                    <div>
                                        ...
                                        <select value={cCategory} onChange={(e) => setCCategory(e.target.value as any)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]">
                                    ...
                                    {cType !== 'text' && (
                                        <div className="md:col-span-2">
                                            ...
                                            <input type="url" required={cType !== 'text'} value={cUrl} onChange={(e) => setCUrl(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]" />
                                        </div>
                                    )}
                                    {cType === 'text' && (
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-bold text-slate-700 mb-1">Text Content *</label>
                                            <textarea required={cType === 'text'} value={cTextContent} onChange={(e) => setCTextContent(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A] min-h-[150px]" />
                                        </div>
                                    )}
                                    {cType !== 'text' && (
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-bold text-slate-700 mb-1">Thumbnail URL</label>
                                            <input type="url" value={cThumbnailUrl} onChange={(e) => setCThumbnailUrl(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]" />
                                        </div>
                                    )}
                                    {cType !== 'text' && (
                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 mb-1">File Size</label>
                                            <input type="text" value={cSize} onChange={(e) => setCSize(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]" />
                                        </div>
                                    )}
                                    {cType === 'video' && (
                                        <div>
                                            <label className="block text-sm font-bold text-slate-700 mb-1">Duration</label>
                                            <input type="text" value={cDuration} onChange={(e) => setCDuration(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#02275A] focus:ring-1 focus:ring-[#02275A]" />
                                        </div>
                                    )}
                                </div>
                            </form>
            ...
            {isAddKbModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm z-[100] animate-fade-in">
                    <div className="bg-white rounded-3xl w-full max-w-3xl shadow-xl overflow-hidden animate-slide-up flex flex-col max-h-[95vh]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h2 className="text-xl font-bold text-slate-800">
                                {editingKbItem ? 'Edit KB Module' : 'Add New KB Module'}
                            </h2>
                            ...
                            <form id="kbForm" onSubmit={handleSaveKb} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="md:col-span-2">
                                        ...
                                        <input type="text" required value={kbTitle} onChange={(e) => setKbTitle(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Description *</label>
                                        <textarea required value={kbDescription} onChange={(e) => setKbDescription(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 min-h-[80px]" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Module Format *</label>
                                        <select value={kbType} onChange={(e) => setKbType(e.target.value as any)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" value={kbDuration} onChange={(e) => setKbDuration(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600" placeholder="e.g. 15 min" />
                                    </div>
                                    
                                    {kbType === 'Video' ? (
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-bold text-slate-700 mb-1">YouTube Embedded URL *</label>
                                            <input type="url" required value={kbUrl} onChange={(e) => setKbUrl(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600" placeholder="https://www.youtube.com/embed/..." />
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <textarea required value={kbTextContent} onChange={(e) => setKbTextContent(e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 min-h-[150px]" />
                                        </div>
                                    )}
                                </div>
                                        ...
                                        {kbQuiz.map((q, i) => (
                                            <div key={q.id} className="bg-slate-50 p-5 rounded-xl border border-slate-200 relative group">
                                                ...
                                                <div className="mb-4 pr-10">
                                                    ...
                                                    <input type="text" required value={q.question} onChange={(e) => handleKbQuizChange(q.id, 'question', e.target.value)} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-600" placeholder="Enter question..." />
                                                </div>
