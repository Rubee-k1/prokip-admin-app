...
const PERMISSION_CATEGORIES: PermissionCategory[] = [
    ...
    },
    {
        ...
        actions: [
            ...
            { id: 'complaints.view', name: 'View Support Tickets' }
        ]
    },
    {
...
const DEFAULT_ROLES: Role[] = [
    ...
    { id: 'role-engineering', name: 'Engineering', type: 'default', description: 'General engineering department staff. Builds and maintains core platform systems and software architectures.', permissions: ['hr.self', 'content.view'], userCount: 2 },
    { id: 'role-engineering-manager', name: 'Engineering Manager', type: 'default', description: 'Manages engineering team sprint backlogs, tech stack decisions, and developer performance evaluation scorecards.', permissions: ['hr.self', 'performance.team', 'content.view'], userCount: 1 },
    { id: 'role-head-engineering', name: 'Head of Engineering', type: 'default', description: 'Highest-level technology role. Oversees all engineering squads, sets architectural direction, decides technical stacks, and evaluates team leads.', permissions: ['hr.self', 'performance.team', 'content.view'], userCount: 1 },
    { id: 'role-ordinary-engineer', name: 'Ordinary Engineer', type: 'default', description: 'Standard software developer responsible for writing clean code, fixing bugs, and implementing features.', permissions: ['hr.self', 'content.view'], userCount: 1 },
...
const getRoleDetails = (roleId: string): { roleName: string; department: string } => {
    switch (roleId) {
        case 'role-super-admin': return { roleName: 'Super Admin', department: 'Admin' };
        case 'role-admin': return { roleName: 'Admin', department: 'Admin' };
        case 'role-head-cx': return { roleName: 'Head of Customer Experience', department: 'Customer Experience' };
        case 'role-sales-manager': return { roleName: 'Sales Manager', department: 'Sales' };
        case 'role-marketing-manager': return { roleName: 'Marketing Manager', department: 'Marketing' };
        case 'role-finance': return { roleName: 'Finance', department: 'Finance' };
        case 'role-content-lead': return { roleName: 'Content Lead', department: 'Content' };
        case 'role-customer-success': return { roleName: 'Customer Success Executive', department: 'Customer Success' };
        case 'role-support-staff': return { roleName: 'Technical Support Specialist', department: 'Technical Support' };
        case 'role-team-lead': return { roleName: 'Team Lead', department: 'Operations' };
        case 'role-engineering': return { roleName: 'Engineer', department: 'Engineering' };
        case 'role-engineering-manager': return { roleName: 'Engineering Manager', department: 'Engineering' };
        case 'role-head-engineering': return { roleName: 'Head of Engineering', department: 'Engineering' };
        case 'role-ordinary-engineer': return { roleName: 'Software Engineer', department: 'Engineering' };
        case 'role-call-agent': return { roleName: 'Call Agent', department: 'Sales' };
        default: return { roleName: 'Employee', department: 'Operations' };
    }
};
...
const AdminUsersView: React.FC = () => {
    ...
    const [users, setUsers] = useState<User[]>(() => {
        ...
        return [
            ...
            { id: '4', employeeId: 'EMP-GH-002', name: 'Kwame Osei', email: 'k.osei@company.com', roleId: 'role-ordinary-engineer', status: 'Active', lastLogin: '2 weeks ago' },
            { id: '5', employeeId: 'EMP-NG-101', name: 'Tunde Bakare', email: 't.bakare@company.com', roleId: 'role-engineering-manager', status: 'Active', lastLogin: '4 mins ago' },
            { id: '6', employeeId: 'EMP-NG-102', name: 'Ngozi Okoye', email: 'n.okoye@company.com', roleId: 'role-engineering', status: 'Active', lastLogin: '12 mins ago' },
            { id: '7', employeeId: 'EMP-MKT-002', name: 'Kola Alabi', email: 'k.alabi@company.com', roleId: 'role-marketing-manager', status: 'Active', lastLogin: 'New' },
            { id: '8', employeeId: 'EMP-CX-002', name: 'Yemi Johnson', email: 'y.johnson@company.com', roleId: 'role-head-cx', status: 'Active', lastLogin: 'New' },
            { id: '9', employeeId: 'EMP-ENG-103', name: 'Nnaemeka Egwu', email: 'n.egwu@company.com', roleId: 'role-head-engineering', status: 'Active', lastLogin: 'New' }
        ];
    });
    ...
    const handleAddUser = (e: React.FormEvent) => {
        ...
        if (editingUserId) {
            ...
            let finalEmployeeId = newUser.employeeId;
            const user: User = {
            ...
            if (!finalEmployeeId) {
                ...
                finalEmployeeId = `EMP-NG-9${Date.now().toString().slice(-3)}`;
                user.employeeId = finalEmployeeId;
                
                // Add new employee to company_employees_data
                const savedHr = localStorage.getItem("company_employees_data");
                let hrList: any[] = [];
                if (savedHr) {
                    try {
                        hrList = JSON.parse(savedHr);
                    } catch (e) {}
                }
                if (hrList.length === 0) {
                ...
                const roleDetails = getRoleDetails(newUser.roleId);
                ...
                const newEmp: any = {
                    ...
                    is_team_lead: newUser.roleId.includes('manager') || newUser.roleId.includes('head') || newUser.roleId.includes('lead'),
                    department_id: `dept-${roleDetails.department.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
                ...
                hrList.push(newEmp);
                localStorage.setItem("company_employees_data", JSON.stringify(hrList));
                localStorage.setItem("company_employees_kpi_state", JSON.stringify(hrList));
            }
