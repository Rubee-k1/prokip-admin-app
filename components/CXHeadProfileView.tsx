...
const CXHeadProfileView: React.FC<CXHeadProfileViewProps> = ({
  setView,
  userRole,
  initialTab,
}) => {
  ...
  if (userRole === "customer-success") {
    ...
    initials = "AE";
    emailStr = "employee@gmail.com";
  }

  const { showSuccess, showError } = useAlert();
  const isSpecialRole = true; // Make all department roles use the standard team lead layout as requested

  // Dynamic defaults based on role to support both CX Head defaults and Employee (Ada Eze) defaults
  const isEmp = userRole === "employee";
  const defFirstName = isEmp ? "Ada" : "CX";
  const defLastName = isEmp ? "Eze" : "Head";
  const defPhone = isEmp ? "+234 802 345 6789" : "+234 812 345 6789";
  const defDob = isEmp ? "1998-09-22" : "1995-05-15";
  const defGender = isEmp ? "Female" : "Male";
  const defMaritalStatus = isEmp ? "Single" : "Single";
  const defCountry = "Nigeria";
  const defStateVal = "Lagos";
  const defCity = "Ikeja";
  const defNin = isEmp ? "11223344556" : "12345678901";
  const defBvn = isEmp ? "22334455667" : "22223333444";
  const defTaxId = isEmp ? "TIN-492-938-2" : "TIN-99887711";
  const defPfaName = "Stanbic IBTC Pension";
  const defPensionNumber = "PEN-00998877";

  // Form fields state for Personal & Employment
  const [firstName, setFirstName] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_first_name") || defFirstName;
    } catch {
      return defFirstName;
    }
  });
  const [lastName, setLastName] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_last_name") || defLastName;
    } catch {
      return defLastName;
    }
  });
  const [phone, setPhone] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_phone") || defPhone;
    } catch {
      return defPhone;
    }
  });
  const [dob, setDob] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_dob") || defDob;
    } catch {
      return defDob;
    }
  });
  const [gender, setGender] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_gender") || defGender;
    } catch {
      return defGender;
    }
  });
  const [maritalStatus, setMaritalStatus] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_marital_status") || defMaritalStatus;
    } catch {
  ...
  const [country, setCountry] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_country") || defCountry;
    } catch {
      return defCountry;
    }
  });
  const [stateVal, setStateVal] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_state") || defStateVal;
    } catch {
      return defStateVal;
    }
  });
  const [city, setCity] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_city") || defCity;
    } catch {
      return defCity;
    }
  });
  ...
  const [nin, setNin] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_nin") || defNin;
    } catch {
      return defNin;
    }
  });
  const [bvn, setBvn] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_bvn") || defBvn;
    } catch {
      return defBvn;
    }
  });
  const [taxId, setTaxId] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_tax_id") || defTaxId;
    } catch {
      return defTaxId;
    }
  });
  const [pfaName, setPfaName] = useState(() => {
    try {
      return (
        localStorage.getItem("cx_profile_pfa_name") || "Stanbic IBTC Pension"
      );
    } catch {
  ...
  const [pensionNumber, setPensionNumber] = useState(() => {
    try {
      return (
        localStorage.getItem("cx_profile_pension_number") || "PEN-00998877"
      );
    } catch {
      return "PEN-00998877";
    }
  ...
  const [emergencyName, setEmergencyName] = useState(() => {
    try {
      return (
        localStorage.getItem("cx_profile_emergency_name") || "Mrs. Sade Sola"
      );
    } catch {
  ...
  const [emergencyPhone, setEmergencyPhone] = useState(() => {
    try {
      return (
        localStorage.getItem("cx_profile_emergency_phone") ||
        "+234 803 111 2222"
      );
    } catch {
  ...
  const [emergencyRelationship, setEmergencyRelationship] = useState(() => {
    try {
      return (
        localStorage.getItem("cx_profile_emergency_relationship") || "Mother"
      );
    } catch {
      return "Mother";
    }
  ...
  const [personalEmail, setPersonalEmail] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_personal_email") || "";
    } catch {
      return "";
    }
  });
  const [placeOfBirth, setPlaceOfBirth] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_place_of_birth") || "";
    } catch {
      return "";
    }
  });
  const [bloodGroup, setBloodGroup] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_blood_group") || "";
    } catch {
      return "";
    }
  });
  const [residentialAddress, setResidentialAddress] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_residential_address") || "";
    } catch {
      return "";
    }
  });
  const [permanentAddress, setPermanentAddress] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_permanent_address") || "";
    } catch {
      return "";
    }
  });
  const [qualification, setQualification] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_qualification") || "";
    } catch {
      return "";
    }
  });
  const [religion, setReligion] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_religion") || "";
    } catch {
      return "";
    }
  });
  const [emergencyAddress, setEmergencyAddress] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_emergency_address") || "";
    } catch {
      return "";
    }
  });
  const [linkedin, setLinkedin] = useState(() => {
    try {
      return localStorage.getItem("cx_profile_linkedin") || "";
    } catch {
      return "";
    }
  });
  ...
  const handleSubmitProfile = (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(
        "cx_profile_emergency_relationship",
        emergencyRelationship,
      );

      // save 9 new fields
      localStorage.setItem("cx_profile_personal_email", personalEmail);
      localStorage.setItem("cx_profile_place_of_birth", placeOfBirth);
      localStorage.setItem("cx_profile_blood_group", bloodGroup);
      localStorage.setItem(
        "cx_profile_residential_address",
        residentialAddress,
      );
      localStorage.setItem("cx_profile_permanent_address", permanentAddress);
      localStorage.setItem("cx_profile_qualification", qualification);
      localStorage.setItem("cx_profile_religion", religion);
      localStorage.setItem("cx_profile_emergency_address", emergencyAddress);
      localStorage.setItem("cx_profile_linkedin", linkedin);
  ...
  const [activeTab, setActiveTab] = React.useState<
    "profile" | "grades" | "history" | "leaderboard"
  >("profile");
  ...
  > = {
    ...
    leaderboard: {
      label: "Leaderboard for department",
      icon: "fa-trophy",
      category: "Performance",
    },
  };
  ...
  const completionPercentage = Math.round((filledFieldsCount / 26) * 100);
      ...
      setUploadProgress((prev) => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
            ...
            setDocDisplayName("");
            showSuccess(
              `Document "${newDoc.name}" uploaded. Sent for approval.`,
            );
          }, 500);
          return 100;
        }
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
              ...
              {Object.entries(subTabDetails)
                .filter(([_, details]) => {
                  const expectedCategory =
                    initialTab === "performance" ? "Performance" : "My Profile";
                ...
                .map(([id, details]) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`w-5.5 h-5.5 rounded-md flex items-center justify-center text-[10px] transition-colors ${
                          isActive
                            ? "bg-white/20 text-white"
                            : "bg-[#02275A]/5 text-[#02275A]"
                        }`}
                      >
                      ...
                      <div>
                        <p
                          className={`text-[11px] font-bold transition-colors ${isActive ? "text-white" : "text-[#02275A]"}`}
                        >
                          {details.label}
                        </p>
                      </div>
        ...
        {!isSpecialRole ? (
          <>
            {activeTab === "profile" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                  <h3 className="font-bold text-[#02275A] text-lg mb-8">
                    Profile Information
                  </h3>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                  <h3 className="font-bold text-[#02275A] text-lg mb-8 flex items-center gap-2">
                    <i className="far fa-lock text-slate-400"></i> Change
                    Password
                  </h3>

                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-bold text-[#02275A] uppercase mb-2">
                        CURRENT PASSWORD *
                      </label>
                      <input
                        type="password"
            ...
            {activeTab === "grades" && (
              <div className="-mt-6">
                <EmployeeGradesRewardsView hideRewards={true} userRole={userRole} />
              </div>
            )}
            {activeTab === "history" && (
              <div className="-mt-6">
                <EmployeeHistoryView setView={setView} userRole={userRole} />
              </div>
            )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs flex flex-col">
                  ...
                  <div className="relative -mt-12 flex flex-col items-center pb-5 px-6">
                    ...
                    <h4 className="text-lg font-extrabold text-[#02275A] text-center">
                      {firstName} {lastName}
                    </h4>
                    ...
                    <div className="w-full mt-6 border-t border-slate-100 pt-5">
                      ...
                      <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
                        <div
                          className="bg-[#02275A] h-full transition-all duration-500"
                    ...
                    <div className="w-full border-t border-slate-100 mt-5 pt-5 space-y-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-xs font-bold text-[#02275A]">
                            {deptName}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 pb-1">
                        ...
                        <div>
                          ...
                          <span
                            className="text-xs font-bold text-[#02275A] truncate max-w-[170px]"
                            title={emailStr}
                          >
                            {emailStr}
                          </span>
                ...
                <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col space-y-6">
                  ...
                  <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-4 flex gap-3 text-xs text-amber-800 leading-relaxed">
                    ...
                    <span>
                      You can fill these details once. After you submit, only HR
                      can change them — so double-check before saving. Your bank
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-slate-50/50 border border-slate-100 p-3.5 rounded-xl flex flex-col justify-center">
                      ...
                      <span className="text-xs font-extrabold text-[#02275A] mt-1">
                        EMP-XX-011
                      </span>
                    </div>
                  ...
                  <form onSubmit={handleSubmitProfile} className="space-y-6">
                    ...
                    <div className="border-t border-slate-100 pt-5">
                      <h4 className="font-bold text-[#02275A] text-sm mb-4 flex items-center gap-2">
                        <i className="far fa-user text-slate-400"></i> Personal
                        Information
                      </h4>
                    ...
                    <div className="border-t border-slate-100 pt-5">
                      <h4 className="font-bold text-[#02275A] text-sm mb-4 flex items-center gap-2">
                        <i className="far fa-life-ring text-slate-400"></i>{" "}
                        Emergency Contact
                      </h4>
                    ...
                    <div className="border-t border-slate-100 pt-5">
                      <h4 className="font-bold text-[#02275A] text-sm mb-4 flex items-center gap-2">
                        <i className="far fa-id-badge text-slate-400"></i>{" "}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-[#02275A] focus:outline-none focus:bg-white focus:border-[#02275A] disabled:bg-slate-100 disabled:text-slate-400 transition-all font-sans"
                          />
                        </div>
                        ...
                        <div className="sm:col-span-2 md:col-span-3">
                          ...
                          <input
                            ...
                            disabled={isSubmitted}
                            placeholder="Enter emergency contact person's address"
                            className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-[#02275A] focus:outline-none focus:bg-white focus:border-[#02275A] disabled:bg-slate-100 disabled:text-slate-400 transition-all"
                          />
                        </div>
                      </div>
                    ...
                    <div className="border-t border-slate-100 pt-5 flex items-center justify-between gap-4">
                      {isSubmitted ? (
                        <div className="flex items-center gap-2 text-emerald-600 font-bold text-xs bg-emerald-50 px-4 py-2.5 rounded-xl border border-emerald-100">
                          <i className="fas fa-check-circle"></i> Profile locked
                          & submitted for compliance checks
                        </div>
                      ) : (
                        <div className="text-[11px] text-slate-400">
                          * Please make sure details match your official
                          documents.
                        </div>
                      )}
                ...
                <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-50/50 border border-slate-100 p-3.5 rounded-xl flex flex-col justify-center">
                      ...
                      <span className="text-xs font-extrabold text-[#02275A] mt-1 font-sans">
                        NGN
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-bold text-[#02275A] uppercase tracking-wider mb-2">
                          BANK NAME
                        </label>
                        <select
                          value={bankName}
                ...
                <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col space-y-6">
                  ...
                  {guarantors.length === 0 ? (
                    ...
                    <div className="space-y-4 pb-1">
                      <h4 className="text-[10px] font-bold text-[#02275A] uppercase tracking-wider">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {guarantors.map((guar, index) => (
                          <div
                            key={index}
                            className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex flex-col gap-2 relative"
                          >
                            <button
                              type="button"
                              onClick={() => {
                                const updated = guarantors.filter(
                                  (_, i) => i !== index,
                                );
                                setGuarantors(updated);
                                try {
                                  localStorage.setItem(
                                    "cx_profile_guarantors",
                                    JSON.stringify(updated),
                                  );
                                } catch {}
                            ...
                            <h4 className="font-bold text-[#02275A] text-xs">
                              {guar.name}
                            </h4>
                            {guar.email && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="Former Professional Supervisor">
                            Former Professional Supervisor
                          </option>
                          <option value="Religious Leader">
                            Religious Leader
                          </option>
                          <option value="Business Mentor">
                            Business Mentor
                          </option>
                        </select>
                      </div>
                ...
                <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {documents.map((doc, index) => (
                          ...
                          >
                            ...
                            <h4
                              className="font-bold text-[#02275A] text-xs truncate"
                              title={doc.name}
                            >
                              {doc.name}
                            </h4>
                  ...
                  {uploadingDoc ? (
                    ...
                    <form onSubmit={handleUploadClick} className="space-y-6">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-[#02275A] focus:outline-none focus:bg-white focus:border-[#02275A] transition-all cursor-pointer"
                          >
                            <option value="ID Card (National ID, Passport, Driver's License)">
                              ID Card (National ID, Passport, Driver's License)
                            </option>
                            <option value="Employment Offer Letter & Agreement">
                              Employment Offer Letter & Agreement
                            </option>
                            <option value="Academic Degree Certificate / CV">
                              Academic Degree Certificate / CV
                            </option>
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-[#02275A] uppercase tracking-wider mb-2">
                          FILE (PDF, JPG, PNG, DOCX · MAX 10MB)
                        </label>
