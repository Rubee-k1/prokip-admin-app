...
const Header: React.FC<HeaderProps> = ({ title, subtitle, setView, unreadCount, notifications, markAsRead, markAllAsRead, toggleMobileMenu, userRole, currentView, logout }) => {
    const [showProfileMenu, setShowProfileMenu] = useState(false);
    const [showNotifMenu, setShowNotifMenu] = useState(false);
    const profileRef = useRef<HTMLDivElement>(null);
    const notifRef = useRef<HTMLDivElement>(null);

    let displayName = 'John Agent';
    let displayRole = 'Elite Agent';
    let profileInitials = 'JA';

    if (userRole === 'admin') {
        displayName = 'Admin';
        displayRole = 'Administrator';
        profileInitials = 'AD';
    } else if (userRole === 'team-lead' || userRole === 'cx-head') {
        displayName = 'CX Head';
        displayRole = 'Customer Experience';
        profileInitials = 'CX';
    } else if (userRole === 'customer-success') {
        displayName = 'CS Head';
        displayRole = 'Customer Success';
        profileInitials = 'CS';
    } else if (userRole === 'sales-manager') {
        displayName = 'Sales Manager';
        displayRole = 'Sales Dept';
        profileInitials = 'SM';
    } else if (userRole === 'marketing-manager') {
        displayName = 'Marketing Manager';
        displayRole = 'Marketing Dept';
        profileInitials = 'MM';
    } else if (userRole === 'support-staff') {
        displayName = 'Support Lead';
        displayRole = 'Support Dept';
        profileInitials = 'SL';
                    ...
                    <div className="relative" ref={profileRef}>
                        ...
                        >
                            <div className="hidden md:block text-right">
                                <p className="text-sm font-bold text-slate-800 group-hover:text-[#02275A] transition-colors leading-tight">{displayName}</p>
                                <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">{displayRole}</p>
                            </div>
                        ...
                        {showProfileMenu && (
                            <div className="absolute right-0 top-14 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-fade-in origin-top-right ring-1 ring-slate-900/5">
                                ...
                                <div className="p-2">
                                    {['call-agent', 'support-staff', 'customer-success', 'team-lead', 'cx-head', 'sales-manager', 'marketing-manager', 'finance', 'content-lead', 'engineering', 'engineer', 'employee'].includes(userRole || '') ? (
                                        <>
                                            <button 
                                                onClick={() => { 
                                        ...
                                        <>
                                            <button 
                                                onClick={() => { 
                                                    if (['cx-head', 'customer-success', 'sales-manager', 'marketing-manager', 'support-staff', 'finance', 'content-lead', 'call-agent', 'engineering', 'engineer', 'employee'].includes(userRole)) {
                                                        setView('team-lead-my-profile');
                                                    } else {
                                                        setView('profile');
                                                    }
                                            ...
                                            {!(userRole === 'employee' && currentView === 'dashboard') && !['cx-head', 'sales-manager', 'content-lead', 'marketing-manager', 'call-agent', 'finance', 'customer-success', 'support-staff', 'engineering', 'engineer'].includes(userRole || '') && (
                                                <button 
                                            ...
                                            {!(userRole === 'employee' && currentView === 'dashboard') && !['cx-head', 'sales-manager', 'marketing-manager', 'finance', 'content-lead', 'engineering', 'engineer'].includes(userRole || '') && (
                                                <button 
