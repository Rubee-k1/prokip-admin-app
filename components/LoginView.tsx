...
const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, onBack, onForgotPassword, onSignUpClick }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (email === 'marketing@gmail.com' && password === '12345') {
            setTimeout(() => {
                setIsLoading(false);
                showSuccess("Welcome, Marketing Lead!");
                onLoginSuccess('marketing-manager', email);
            }, 1500);
            return;
        }

        // Marketer Login Check
        if ((email === 'marketer@gmail.com' || email === 'marketer') && password === '12345') {
            setTimeout(() => {
                setIsLoading(false);
                showSuccess("Welcome, Marketer!");
                onLoginSuccess('employee', 'marketer@gmail.com');
            }, 1500);
            return;
        }
