...
const ManagerLeadsView: React.FC<ManagerLeadsViewProps> = ({ setView, businesses = [], invoices: propInvoices = [], onAddInvoice, onUpdateInvoice, onAddNotification, initialTab = 'list', userCountry = 'Nigeria' }) => {
    const { showSuccess, showInfo } = useAlert();
    const [activeTab, setActiveTab] = useState<'list' | 'invoices' | 'find' | 'appointment'>(initialTab);
    
    // Mock Agents Data (Call Agents)
    ...
    // Lead List State
    const [activeCategory, setActiveCategory] = useState<'All' | 'Personal' | 'Company'>('All');
    ...
    const [leads, setLeads] = useState<Lead[]>([
        ...
        { id: 2, name: "Madam Sarah", business: "Sarah Salon", type: "Personal", status: "Meeting Scheduled", phone: "08022211100", location: "Garki 2", lastAction: "Visit (Yesterday)", notes: [{ date: '2023-10-20', type: 'Visit', text: 'Visited shop. Demonstrated POS.' }], reminders: [], assignedAgentId: 'AG-002', assignedAgentName: 'Emmanuel K.' },
        { id: 3, name: "TechPoint Logistics", business: "TechPoint", type: "Company", status: "New", phone: "08199988877", location: "Central Area", lastAction: "None", notes: [] }, // Unassigned
        { id: 4, name: "Bisi Cakes", business: "Bisi Bakery", type: "Personal", status: "Negotiating", phone: "07055566677", location: "Maitama", lastAction: "Email (Today)", notes: [{ date: '2023-10-25', type: 'Note', text: 'Sent pricing proposal.' }], reminders: [], assignedAgentId: 'AG-001', assignedAgentName: 'Sarah O.' },
        { id: 5, name: "BlueChip Inc.", business: "BlueChip", type: "Company", status: "Converted", phone: "09011122233", location: "Wuse 2", lastAction: "Onboarding", notes: [{ date: '2023-10-15', type: 'System', text: 'Lead converted to paid customer.' }], reminders: [], assignedAgentId: 'AG-003', assignedAgentName: 'Chidinma N.' },
        { id: 6, name: "Emeka Phones", business: "Emeka Phones", type: "Personal", status: "New", phone: "08012312312", location: "Banex Plaza", lastAction: "None", notes: [] },
        { id: 7, name: "Kiddies World", business: "Kiddies World", type: "Company", status: "Interested", phone: "07098798798", location: "Gwarinpa", lastAction: "Call (1 week ago)", notes: [], assignedAgentId: 'AG-002', assignedAgentName: 'Emmanuel K.' },
    ]);
    ...
    const filteredLeads = leads
        .filter(l => {
            ...
            const matchesStateManager = stateManagerFilter === 'All' || l.managerId === stateManagerFilter; // Using managerId loosely for demo
            const matchesBrm = brmFilter === 'All' || l.managerId === brmFilter;
            ...
            return matchesCategory && matchesSearch && matchesStatus && matchesAgent && matchesStateManager && matchesBrm;
        })
        .sort((a, b) => {
    ...
    const handleCategoryChange = (category: 'All' | 'Personal' | 'Company') => {
                ...
                <div className="animate-fade-in">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto overflow-x-auto no-scrollbar pb-1">
                            ...
                            <div className="flex bg-white p-1 rounded-full border border-slate-200 shrink-0">
                                <button 
                                    onClick={() => { setAgentFilter('All'); setCurrentPage(1); }}
                                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${agentFilter === 'All' ? 'bg-[#02275A] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                                >
                                    All Leads
                                </button>
                                <button 
                                    onClick={() => { setAgentFilter('Assigned'); setCurrentPage(1); }}
                                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${agentFilter === 'Assigned' ? 'bg-[#02275A] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
                                >
                                    Assigned to Telesales
                                </button>
                            </div>
                            ...
                            <div className="relative shrink-0">
                                <select 
                                    ...
                                    onChange={(e) => { setStateManagerFilter(e.target.value); setCurrentPage(1); }}
                                    className={`appearance-none pl-3 pr-8 py-2 rounded-full text-xs font-bold border transition-all cursor-pointer outline-none focus:ring-2 focus:ring-[#02275A]/20 ${stateManagerFilter !== 'All' ? 'bg-[#02275A] text-white border-[#02275A]' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}
                                >
                                ...
                                <i className={`fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-[10px] pointer-events-none ${stateManagerFilter !== 'All' ? 'text-white' : 'text-slate-400'}`}></i>
                            </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
                            ...
                            <button onClick={() => setIsAddModalOpen(true)} className="bg-[#02275A] text-white px-4 py-2 rounded-lg text-xs font-bold shadow-md hover:opacity-90 transition-all active:scale-95 flex items-center gap-2 justify-center flex-1 md:flex-none whitespace-nowrap">
                                        ...
                                        <div className="flex items-start gap-3 flex-1">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                                                    ...
                                                    {lead.managerName && (
                                                        <span className="text-[10px] bg-sky-50 text-sky-700 px-2 py-0.5 rounded flex items-center gap-1 font-bold" title={brmsList.find(b => b.id === lead.managerId)?.address || ''}>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto mb-4">
                            {callAgents.filter(a => a.name.toLowerCase().includes(agentSearchTermModal.toLowerCase())).map(agent => (
                                <button 
                                    key={agent.id}
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto mb-4">
                            {brmsList.filter(b => b.name.toLowerCase().includes(brmSearchTermModal.toLowerCase()) || b.address.toLowerCase().includes(brmSearchTermModal.toLowerCase())).map(brm => (
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${leadToAssign.brmId === brm.id ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-slate-100 hover:border-emerald-200 hover:bg-slate-50'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left w-full">
                                            <p className="text-sm font-bold text-slate-800 max-w-40 truncate">{brm.name}</p>
                                            <p className="text-[10px] text-slate-500 truncate text-wrap max-w-48 leading-tight">{brm.address}</p>
                                        </div>
                                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={(e) => { 
                            ...
                            const assignedAgentName = assignedAgentId ? callAgents.find(a => a.id === assignedAgentId)?.name : undefined;
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select 
                                        disabled
                                        className="w-full p-2.5 border border-slate-200 bg-slate-50 rounded-lg text-sm mt-1 focus:outline-none cursor-not-allowed"
                                        value="Company"
                                    >
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase">Lead Category</label>
                                    <select 
                                        name="leadCategory" 
                                        required 
                                        className="w-full p-2.5 border border-slate-200 bg-white rounded-lg text-sm mt-1 focus:outline-none focus:border-[#02275A]"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase">Owner (Telesales)</label>
                                    <select name="assignedAgentId" className="w-full p-2.5 border border-slate-200 rounded-lg text-sm mt-1 focus:outline-none focus:border-[#02275A]">
                                        <option value="">Select Agent (Self)</option>
                                        {callAgents.map(a => (
                                            <option key={a.id} value={a.id}>{a.name}</option>
                                        ))}
                                    </select>
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase">Assign to BRM</label>
                                    <div className="mt-1">
                                        <input 
                                            type="text" 
                                            placeholder="Search BRM by address..." 
                                            className="w-full p-2.5 border border-slate-200 rounded-t-lg text-sm focus:outline-none focus:border-[#02275A]"
                                            onChange={(e) => {
                                        ...
                                        />
                                        <select name="brmId" className="w-full p-2.5 border border-t-0 border-slate-200 rounded-b-lg text-sm focus:outline-none focus:border-[#02275A]">
                                            <option value="">Select BRM</option>
                                            {brmsList.map(brm => (
                                                <option key={brm.id} value={brm.id} data-search={`${brm.name} ${brm.address}`}>{brm.name} - {brm.address}</option>
                                            ))}
                                        </select>
                                    </div>
                                </div>
...
export default ManagerLeadsView;
