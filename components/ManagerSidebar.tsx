...
const ManagerSidebar: React.FC<ManagerSidebarProps> = ({ currentView, setView, isMobileMenuOpen, setIsMobileMenuOpen, logout }) => {
    const menuItems = [
        { id: 'manager-dashboard', icon: 'fa-chart-pie', label: 'Overview' },
        { id: 'manager-agents', icon: 'fa-users', label: 'Manage Agents' },
        { id: 'manager-leads', icon: 'fa-filter', label: 'Leads' },
        { id: 'manager-earnings', icon: 'fa-wallet', label: 'Earnings & Wallet' },
        { id: 'manager-performance', icon: 'fa-chart-line', label: 'Performance' },
        { id: 'manager-reports', icon: 'fa-file-alt', label: 'Advanced Reports' },
    ];
        ...
        <>
            ...
            <nav className={`fixed top-0 left-0 h-full w-[264px] bg-white border-r border-slate-200 z-50 transform transition-transform duration-300 ease-in-out flex flex-col shadow-2xl lg:shadow-none lg:translate-x-0 lg:static ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 flex items-center justify-center">
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <defs>
                                    <linearGradient id="manager-gold-gradient" x1="13" y1="10" x2="28" y2="30" gradientUnits="userSpaceOnUse">
                                ...
                                <path 
                                    ...
                                    stroke="url(#manager-gold-gradient)" 
                                    strokeWidth="5" 
                                    strokeLinecap="round" 
                        ...
                        <div>
                            ...
                            <span className="text-[10px] text-[#02275A] font-bold uppercase tracking-widest mt-1 block">Manager Portal</span>
                        </div>
                    </div>
                ...
                <div className="flex flex-col w-full px-4 overflow-y-auto custom-scrollbar flex-1 gap-2 pt-4">
                    <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Management</p>
                    {menuItems.map(item => (
                        <button 
                            key={item.id}
