...
const TeamLeadSidebar: React.FC<TeamLeadSidebarProps> = ({ currentView, setView, isMobileMenuOpen, setIsMobileMenuOpen, logout, userRole }) => {
    let menuItems = [
        ...
        { id: 'team-lead-my-grades', icon: 'fa-star', label: 'Reward' },
    ...
    if (isCxEquivalent) {
        menuItems = menuItems.filter(item => item.id !== 'team-lead-my-grades' && item.id !== 'team-lead-my-history' && item.id !== 'team-lead-my-policies');
    }

    const teamLeadRolesToRemoveMyPerformance = []; // Keep it visible for equivalent roles
    if (userRole && teamLeadRolesToRemoveMyPerformance.includes(userRole)) {
        menuItems = menuItems.filter(item => item.id !== 'team-lead-my-performance');
    }
    ...
    let teamMenuItems = [
        ...
        { id: 'team-lead-complaints', icon: 'fa-ticket', label: 'Support Tickets' },
    ...
    if (userRole === 'support-staff' || userRole === 'customer-success') {
    ...
    if (userRole === 'sales-manager' || userRole === 'marketing-manager') {
        ...
        if (userRole === 'sales-manager') {
            teamMenuItems.push({ id: 'manager-earnings', icon: 'fa-wallet', label: 'Earnings & Wallet' });
            teamMenuItems.push({ id: 'admin-discounts', icon: 'fa-tags', label: 'Discount Management' });
        }
    }
    ...
    if (userRole === 'engineering' || userRole === 'engineer') {
        teamMenuItems = teamMenuItems.filter(item => item.id === 'team-lead-performance' || item.id === 'team-lead-knowledgebase');
        teamMenuItems.push({ id: 'team-lead-resources', icon: 'fa-folder-open', label: 'Resource Center' });
    }
