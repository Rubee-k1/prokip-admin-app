...
// First let's check if my previous state injection worked.
if (!code.includes("const [templates, setTemplates]")) {
    code = code.replace(
...
if (!code.includes("const handleCreateTemplate = () => {")) {
    code = code.replace(
...
const parts1 = code.split("{selectedTemplate && (");
if (parts1.length === 2) {
    const parts2 = parts1[1].split("const renderCreateBroadcast = () => (");
    if (parts2.length === 2) {
    ...
    `;
        ...
        code = parts1[0] + "{selectedTemplate && (" + replacementStr + "const renderCreateBroadcast = () => (" + parts2[1];
        fs.writeFileSync('components/AdminBroadcastsView.tsx', code);
        console.log("Replaced successfully!");
    } else {
        console.log("Could not find const renderCreateBroadcast = () => (");
    }
} else {
    console.log("Could not find {selectedTemplate && (");
}
